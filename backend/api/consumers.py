...
def get_action_space_shape(action_space):
    """Helper to get the shape of a gymnasium action space."""
    if hasattr(action_space, 'n'):  # Discrete space
        return action_space.n
    elif hasattr(action_space, 'shape'):  # Continuous space
...
class AgentConsumer(AsyncWebsocketConsumer):
        ...
        try:
            ...
            if session_id == "new":
                ...
                if db_session:
                    ...
                    action_space_shape = get_action_space_shape(self.env.action_space) if self.env else None
                    response = {
                        "type": "agent.joined",
    ...
    async def handle_agent_action(self, action_data):
        ...
        try:
            ...
            # For Atari games, 'terminated' is True when a life is lost. The game is over when lives are 0.
            is_atari = 'ale-py' in getattr(self.env.spec, 'id', '')
            if is_atari:
                game_over = info.get('lives') == 0 or truncated
            else:
            ...
            unified_data = {
                ...
                "observation": observation,
                "reward": reward,
                "done": game_over,  # Use game_over for the 'done' flag
            ...
            await self._update_session_cache(observation, reward, game_over, info)

            if game_over:
                logger.info(f"Episode completed for session {self.session_id}")
                await self.broadcast_game_over(self.total_reward, self.steps)
    ...
    async def environment_reset_message(self, event):
        """
        Handles a request to reset the environment from another part of the application.
        """
        if not self.env:
            await self.send_error("Environment not initialized", "ENV_NOT_INITIALIZED")
            return
        ...
        try:
            ...
            self.total_reward = 0.0
            self.steps = 0
            logger.info(f"Environment reset for session {self.session_id} via channel layer.")
            ...
            # Broadcast the reset event to all clients in the group
            await self.channel_layer.group_send(
                self.room_group_name,
                {
                    "type": "session_update",
...
class DayTraderDataConsumer(AsyncWebsocketConsumer):
    ...
    async def receive(self, text_data):
        try:
            ...
            if message_type == 'daytrader_action':
                ...
                await self.handle_get_observation(data)
            else:
    ...
    async def handle_get_observation(self, data):
        """Handle request for current observation."""
        try:
            session_id = data.get('session_id')
            if not session_id:
                await self.send(text_data=safe_json_dumps({
                    "type": "error",
                    "message": "session_id is required for get_observation"
                }))
                return

            session_data = cache.get(f"gym_session_{session_id}")
            if not session_data:
                await self.send(text_data=safe_json_dumps({
                    "type": "error",
                    "message": "Environment not found for the given session_id"
                }))
                return

            # The observation is already stored in the session_data dictionary
            observation = session_data.get("current_observation")
            await self.send(text_data=safe_json_dumps({
            ...
            logger.error(f"[v0] Error in handle_get_observation: {e}", exc_info=True)
            await self.send(text_data=safe_json_dumps({
                "type": "error",
                "message": str(e)
            }))
    ...
    async def handle_agent_join(self, data):
        ...
        try:
            ...
            if not session_exists:
                ...
                await self.send(text_data=json.dumps(response, cls=NumpyEncoder))
