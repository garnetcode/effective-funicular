...
# in Section 3.1 of the Project Chimera specification.
...
class GNG_Engine:
    def __init__(self, dimensions, **kwargs):
        ...
        # Node List: { node_id: {'weight': np.array, 'error': float} }
        self.nodes = {}
    ...
    def process_input(self, input_vector):
        ...
        # 3. Adaptation
        # Move winner and its direct neighbors towards the input vector
        ...
        for n_id in neighbor_ids:
            self.nodes[n_id]['weight'] += self.neighbor_learning_rate * (input_vector - self.nodes[n_id]['weight'])
    ...
    def _insert_node(self):
        ...
        # Initialize error of new node r
    ...
    def get_state(self):
        ...
        serializable_nodes = {
            nid: {'weight': node['weight'].tolist(), 'error': node['error']}
    ...
    def from_state(cls, state_dict, **kwargs):
        ...
        gng.nodes = {
            int(nid): {'weight': np.array(node['weight']), 'error': node['error']}
